...
>>>>>>> 2c5423e (변경 파일 커밋)
  ...
  // 사용자 브랜드 설정 로드
  useEffect(() => {
    const loadUserBrandPreferences = () => {
      const savedBrands = localStorage.getItem('userSelectedBrands');
      if (savedBrands) {
        try {
          const brandArray = JSON.parse(savedBrands);
          setFilters((prev) => ({ ...prev, brand: brandArray }));
        } catch (error) {
...
=======
        <Route path="/hotel" element={<AccommodationList />} />
...
=======
        <Route path="/admin" element={<AdminLayout />}>
          <Route index element={<Navigate to="/admin/report" replace />} />
